...
class SerialMaster(object):
	...
	def __init__(self, device="/dev/ttyUSB0", baudrate=19200, parity="N", databytes=8, stopbits=1, keyon=0.0, keyoff=0.0, timeout=3.0, host_group=3, host_address=1):
		self.device = device
	...
	def _do_open(self):
		"""Connect to the ROC slave"""
		
		self._sock = serial.Serial(port=self.device, baudrate=self.baudrate, bytesize=self.databytes, parity=self.parity, stopbits=self.stopbits,timeout=self.timeout)
		#self._sock.setTimeout(self.timeout) # NON-BLOCKING
		
		self._sock.flushInput()
		self._sock.flushOutput()
		try:
			self._sock.close()
		except:
			pass
	...
	def _do_close(self):
		"""Close the connection with the ROC Slave"""
		if self._sock:
			self._sock.close()
			self._sock = None
		return True
